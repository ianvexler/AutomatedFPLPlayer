...
class PlayerMatcher:
  def __init__(self):
    ...
    self.COLUMNS = ["team", "name", "goals", "assists"]
  ...
  def _normalize_fpl_data(self, data):
    ...
    required_columns = ["id", "web_name"] + self.COLUMNS
  ...
  def get_fbref_data(self, season):
    ...
    players_df = self._normalize_fbref_data(players_df)
    return players_df

  def _normalize_fbref_data(self, data):
  ...
  def create_player_dict(self, start_year=20):
    ...
    for start_year in range(start_year, 25):
      ...
      for _, player_data in fbref_data.iterrows():
        ...
        self.player_dict.setdefault(player_name, { 
          "FBref": {},
          "FPL": {}
        })

        # By default add FBref 
        self.player_dict[player_name]["FBref"] = {
          "name": player_name
        }
        ...
        self.player_dict[player_name]['FPL'][season] = {
          "id": fpl_id,
  ...
  def find_fpl_closest_match(self, fbref_player_data, fpl_data, season):
    ...
    fpl_team_ids = [
      ...
      for team_data in team_mapping.load_dict_from_json().values() 
      if team_data['FBref'] in (fb_teams) and season in team_data["FPL"]
    ]
  
    fpl_data = fpl_data[fpl_data['team'].isin(fpl_team_ids)]
    
    # Find the best matching name in fpl_data
    match_results = process.extract(fb_name, fpl_data["name"], scorer=fuzz.token_sort_ratio, limit=10)
    ...
    web_name_match_results = process.extract(fb_name, fpl_data["web_name"], scorer=fuzz.token_sort_ratio, limit=10)
